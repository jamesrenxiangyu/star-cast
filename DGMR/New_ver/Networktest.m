...
result_con = get_GPCA(Cluster, NUM, Ch_par, QoS_th, refDis, src_id);
...
for j = 2:NUM
    ...
    temp_relay_cost = 0;
    for clu = 1:size(relay_cost, 1)
        % use aggregated e2e cost of front and back transmission
        temp_relay_cost = temp_relay_cost + relay_cost{clu,1}(end);
    end
    % remove redudant V2N2V cost
    temp_relay_cost = temp_relay_cost - (clu-1) * Cdelay;
    bar(j, temp_relay_cost);
    hold on;
