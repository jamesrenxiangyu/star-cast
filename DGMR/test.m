...
% Filter out single member cluster
num = []; % Cluster id using multihop multicast V2V
numUnicast = []; % Cluster id using unicast V2I
for k = 1 : nCluster
    kk = nonzeros(Cluster_loc(k,:));
    if length(kk) == 1
        numUnicast = [numUnicast; k];
        continue;
    end
    num = [num; k];
end
...
for i = 1:NUM
    fg = find(Cluster_loc(num(i), 2:end) == 0, 1);
    tempLoc = Cluster_loc(num(i), 1:fg) - Cluster_loc(num(i), 1); % scalarize location of each cluster
    tempCov = Cluster_loc(num(i), fg) - Cluster_loc(num(i), 1);
...
%         [cost, rCan] = dijkstra_v2(g1, g2, 1, length(tempLoc), QoS_th);
        ...
        dLoc = Cluster_loc(num(i),1) + tempLoc(rCan);
        ...
        dTable = distTable(rCan);
        dLoc = Cluster_loc(num(i), 1) + table2Loc(dTable);
        dLoc = dLoc';
        [pdr, rate] = linkCal(dTable, Ch_par);
        rel = prod(pdr); % e2e reliability
        delay = sum(Pkt./rate + Tau * ones(size(pdr,1),1)) * 1e3; % e2e delay
        cost = 0;
    end
...
figure(1)
for j = 1:NUM
    jj = find(Cluster_loc(num(j), 2:end) == 0, 1);
    plot(linspace(Cluster_loc(num(j), 1), Cluster_loc(num(j), jj), length(Cluster_loc(num(j), 1:jj))), 'r-');
    hold on;
    a = loc_con(j,2:end);
    flag = find( a == 0, 1);
