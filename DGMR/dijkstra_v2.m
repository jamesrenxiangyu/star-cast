%% GPCA: Modified Dijkstra:
% Takes in two graphs: Graph1 contains the delay_cost and use original
% Dijkstra algorithm; Graph2 contains the pdr and is used to filter out
% error nodes
function [Cost, Route] = dijkstra_v2(Graph1, Graph2, SourceNode, TerminalNode, QoS_th)
...
if size(Graph1,1) ~= size(Graph1,2)
   fprintf('The Graph must be a square Matrix\n');
   return; 
elseif min(min(Graph1)) < 0
    fprintf('Dijkstras algorithm cannot handle negative costs.\n')
    fprintf('Please use Bellman-Ford or another alternative instead\n');
    return;
elseif SourceNode < 1 && (rem(SourceNode,1)==0) && (isreal(SourceNode)) && (SourceNode <= size(Graph1,1))
    fprintf('The source node must be an integer within [1, sizeofGraph]\n');
    return;
elseif TerminalNode < 1 && (rem(TerminalNode,1)==0) && isreal(TerminalNode) && (TerminalNode <= size(Graph1,1))
...
if SourceNode == TerminalNode
    Cost = Graph1(SourceNode, TerminalNode);
    Route = SourceNode;
    return;
end
...
% that)
PathToNode = cell(size(Graph1,1),1);

% Initialize all Node costs to infinity except for the source node
NodeCost = Inf.*ones(1,size(Graph1,1));
...
while (CurrentNode ~= TerminalNode)
    ...
    for i = 1:length(UnvisitedNeighborsCostVector)
       if (UnvisitedNeighborsCostVector(i) ~= Inf) % Only Check for update if non-infinite 
           temp_rel_flag = UnvisitedNeighborsRelVector(i);
           ...
           % assign the minimum
           if tempCost < NodeCost(UnvisitedNodes(i)) && temp_rel_flag >= rel_th
               NewPathToNeighbor = [PathToCurrentNode(PathToCurrentNode~=0) CurrentNode]; % The new available path to get to the neighbor 
               NewPath = [NewPathToNeighbor zeros(1,size(Graph1,1)-size(NewPathToNeighbor,2))];
               PathToNode{UnvisitedNodes(i)}(:) = NewPath;
               NodeCost(UnvisitedNodes(i)) = tempCost;
           elseif tempCost < NodeCost(UnvisitedNodes(i)) && temp_rel_flag < rel_th
                temp_rel_flag
           end
        end
    end
